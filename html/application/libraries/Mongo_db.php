...
class Mongo_db
{
	...
	*/
	...
	function mongoId($id)
	{
		if (isset($id->{'$id'}) == TRUE) return $id;
		else return new MongoId($id);
	}

	function find($table, $query = array(), $return_cursor = TRUE)
	{
		$cursor = $this->_dbhandle->{$table}->find($query);
	...
	function findOne($table, $query = array())
	{
		return $this->_dbhandle->{$table}->findOne($query);
		// $cursor = $this->_dbhandle->{$table}->findOne($query);
	...
	*/
	...
	function update_set($table, $criteria, $newobj, $options = null)
	{
		try
		{
			if ($options === true)
			...
			else if ($options == null || is_array($options) == FALSE)
			{
				$options = array('safe' => true);
			}
			
			$output = $this->_dbhandle->{$table}->update($criteria, array('$set' => $newobj), $options);
			
			if (isset($output['n']) == true && $output['n'] > 0)
			{
				return $output['n'];
			}
			else
		...
		{
			dpException::showError('An error has occurred. (Update from MongoDB)');
	...
	function update_set_multiple($table, $criteria, $newobj)
	{
		try
		{
